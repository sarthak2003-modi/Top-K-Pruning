# %% =============================== #
#  VGG8 on CIFAR-10 @ 32x32 - OPTIMIZED VERSION
...
#  
#  VGG8 architecture: 4 conv blocks (64-64-128-128) + 2 FC layers
# %% =============================== #
import argparse
import math
import random
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='VGG8 sparse training on CIFAR-10 (optimized)'
    )
    ...
    parser.add_argument('--batch-train', type=int, default=128,
                        help='Training batch size (default: 128)')
    parser.add_argument('--batch-val', type=int, default=256,
    ...
    parser.add_argument('--pretrain-path', type=str, default=None,
                        help='Path to pretrained VGG8 checkpoint')
...
# ----------------- VGG8 Architecture for CIFAR-10 -----------------
class VGG8(nn.Module):
    """VGG8 for CIFAR-10 (32x32 input)."""
...
# ----------------- Model loading -----------------
def load_vgg8_cifar10(pretrained=True, pretrain_path=None):
    """
    Load VGG8 for CIFAR-10.
    
    Args:
        pretrained: If True, attempt to load pretrained weights
        pretrain_path: Optional path to custom pretrained checkpoint
    
    Returns:
        VGG8 model
    """
    model = VGG8(num_classes=10)
    ...
    if pretrained:
        if pretrain_path is not None:
            ...
            print("=" * 70)
            print("NOTE: Training VGG8 from scratch (no pretrained weights available)")
            print("VGG8 is a custom lightweight architecture for CIFAR-10")
            print("Expected baseline accuracy: ~91-92%")
            print("=" * 70 + "\n")
    else:
...
if __name__ == '__main__':
    ...
    print("Configuration:")
    print(f"  Model: VGG8")
    print(f"  Dataset: CIFAR-10 (32x32)")
    ...
    # Data (CIFAR-10 @ 32x32, standard normalization)
    CIFAR10_MEAN = (0.4914, 0.4822, 0.4465)
    CIFAR10_STD = (0.2470, 0.2435, 0.2616)
    
    # CIFAR-10 augmentation (matching reference code)
    ...
    val_set = datasets.CIFAR10(root=args.data_root, train=False, download=True, transform=val_tfms)
    ...
    # Save final model
    print("\nSaving final sparse model...")
    torch.save({
        'model_state_dict': model.state_dict(),
        'topk': args.topk,
        'reg_type': args.reg_type,
        'final_accuracy': val["top1"],
    }, 'vgg8_sparse_final.pth')
    print("Model saved to: vgg8_sparse_final.pth")
